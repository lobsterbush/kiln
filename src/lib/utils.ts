 ...
 *  Falls back to a manual implementation for browsers without crypto.randomUUID
 *  (e.g. older Safari, non-HTTPS contexts). */
export function generateToken(): string {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return crypto.randomUUID()
  }
  // Fallback: construct a v4-style UUID from getRandomValues
 ...
 * Array.sort(random) is biased because its comparator is non-transitive.
 */
